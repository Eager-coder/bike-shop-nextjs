...
export default checkAuth(async (req, res) => {
	if (!req.method === "POST")
		return res
			.status(401)
			.json({ message: "We only support POST!", isSuccess: false })
	...
	if (userData.type === "profile") {
		...
		if (results.email !== newEmail) {
			...
			if (existingEmail)
				return res
					.status(401)
					.json({ message: "Email is already registered!", isSuccess: false })
			await db.query(
		...
		res
			.status(200)
			.json({ message: "Profile has been changed!", isSuccess: true })
		...
		if (newPassword !== confirmPassword)
			return res
				.status(401)
				.json({ message: "Passwords must match!", isSuccess: false })
		...
		if (!isMatch)
			return res
				.status(401)
				.json({ message: "Password is not correct", isSuccess: false })
		...
		return res
			.status(200)
			.json({ message: "Password has been changed", isSuccess: true })
	} else {
